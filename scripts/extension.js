    ...
    button.addEventListener('click', function () {
        ...
        chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
          if (tabs[0]['url'].includes('kotsovolos') || tabs[0]['url'].includes('ab.gr')){
            chrome.tabs.sendMessage(tabs[0].id, {type: "marketplace"}, function(response) {
          ...
          else if (tabs[0]['url'].includes('booking')){
            console.log('test-booking')
            chrome.tabs.sendMessage(tabs[0].id, {type: "book-accommodation"}, function(response) {
                provider = response.provider
                // Create the list element:
                console.log(response.accommodation)
                $('#product_list').html(response.accommodation);
                var table = document.createElement('table');
                table.id = 'product_list'
                var row = document.createElement('tr');
                var item = document.createElement('td');
